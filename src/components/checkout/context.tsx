...
export const CheckoutProvider = (props: any) => {
  ...
  const setCardFields = useCallback((card: CardFields) => dispatch({ type: 'SET_CARD_FIELDS', card }), [dispatch])
  ...
  const clearCheckoutFields = useCallback(() => dispatch({ type: 'CLEAR_CHECKOUT_FIELDS' }), [dispatch])
