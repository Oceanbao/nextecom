...
import { defaultOperations, OPERATIONS, AllOperations, APIOperations } from './operations'
...
export type Endpoint<C extends CommerceAPI, E extends EndpointSchemaBase> = APIEndpoint<
  C,
...
export class CommerceAPICore<P extends APIProvider = APIProvider> {
  ...
  getConfig(userConfig: Partial<P['config']> = {}): P['config'] {
    return Object.entries(userConfig).reduce((cfg, [key, value]) => Object.assign(cfg, { [key]: value }), {
...
export function getCommerceApi<P extends APIProvider>(customProvider: P): CommerceAPI<P> {
  const commerce = Object.assign(new CommerceAPICore(customProvider), defaultOperations as AllOperations<P>)
...
export type GraphQLFetcher<Data extends GraphQLFetcherResult = GraphQLFetcherResult, Variables = any> = (
  query: string,
  queryData?: CommerceAPIFetchOptions<Variables>,
  fetchOptions?: FetchOptions
) => Promise<Data>
